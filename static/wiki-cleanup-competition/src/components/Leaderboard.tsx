...
const LeaderboardSkeleton: React.FC = () => (
  <div className="space-y-3 animate-pulse">
    {[...Array(5)].map((_, i) => (
      <div
        key={i}
        className="flex items-center p-3 h-[60px] bg-nr-dark-light/50 rounded-lg"
      >
...
const EligibleUserList: React.FC<{ title: string; users: User[] }> = ({
  title,
  users,
}) => (
  <div>
    ...
    <ul className="space-y-2">
      {users.length > 0 ? (
        users.slice(0, 10).map((user, index) => (
          <li
            key={user.username}
            className="flex items-center bg-nr-dark-light/50 p-2 rounded-md"
          >
      ...
      <section className="bg-nr-dark-card rounded-lg shadow-lg p-6">
        ...
        <div className="bg-nr-dark-light/50 rounded-lg p-6 flex flex-col items-center text-center">
          ...
          <p className="text-gray-300 mt-2 max-w-md">
            The <strong>$250 grand prize</strong> winner will be randomly drawn
            from the top 10 editors of Week 1 and Week 2.
          </p>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="bg-nr-dark-light/30 rounded-lg overflow-hidden border border-nr-dark-light/50">
            <div className="grid grid-cols-12 gap-4 p-3 bg-nr-dark-light/50 text-xs font-bold text-gray-400 uppercase tracking-wider">
              <div className="col-span-2 sm:col-span-1 text-center">Rank</div>
              <div className="col-span-7 sm:col-span-9">User</div>
            ...
            <div className="max-h-96 overflow-y-auto">
              {users.map((user, index) => (
                <div
                  key={user.username}
                  className="grid grid-cols-12 gap-4 p-3 border-t border-nr-dark-light/20 items-center hover:bg-nr-dark-light/30 transition-colors"
                >
                  <div className="col-span-2 sm:col-span-1 text-center font-mono text-gray-500">
                    {index + 1}
                  </div>
                  <div className="col-span-7 sm:col-span-9 flex items-center">
                    <img
                      src={user.avatar}
                  ...
                  <div className="col-span-3 sm:col-span-2 text-right font-bold text-nr-green-accent">
        ...
        <div className="space-y-2">
          {users.length > 0 ? (
            users.slice(0, 10).map((user, index) => (
              <div
                key={user.username}
                className="bg-nr-dark-light/50 rounded-lg transition-colors duration-200 hover:bg-nr-dark-light"
              >
                <div
                ...
                >
                  ...
                  />
                  <span className="font-medium flex-grow text-nr-font">
                    {user.name}
                  </span>
                  <span className="font-bold text-nr-green-accent text-lg mr-4">
                    {Math.round(user.score)} pts
                  </span>
                  <ChevronDownIcon
                    className={`h-5 w-5 text-gray-400 transform transition-transform duration-200 ${
                      expandedUser === user.username ? "rotate-180" : ""
                    }`}
                  />
                </div>
                ...
                <div
                  ...
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    expandedUser === user.username ? "max-h-96" : "max-h-0"
                  }`}
                >
                  <div className="pl-16 pr-4 pb-3">
                    <ul className="space-y-2 text-sm border-l-2 border-nr-dark-light pl-4">
                      {updates
                        .filter(
                          (update) => update.user.username === user.username
                        )
                        .map((update) => (
                          <li
                            key={update.id}
                          ...
                          >
                            <div className="flex items-center truncate">
                              <a
                                ...
                                className="truncate hover:underline hover:text-nr-green-accent"
