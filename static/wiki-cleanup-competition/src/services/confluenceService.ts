...
export const shouldExcludePage = (title: string): boolean => {
  if (title === "GTS Wiki Cleanup Change Log") {
    console.log(`[Exclusion] Excluding page: "${title}"`);
    return true;
  }
  if (title.includes("Pairing")) {
    console.log(`[Exclusion] Excluding page: "${title}"`);
    return true;
  }
  return false;
...
const applyBonuses = (
  ...
  return updates.map((update) => {
    ...
    const userSession = bonusSessions.find((session) => {
      ...
      const emailMatch =
        authorEmail.startsWith(sessionUser) ||
        authorEmail.startsWith(sessionUser + "@");
      ...
      if (
        ...
        console.log(
          `[Bonus] Matched session user "${session.user}" to author "${update.user.name}" (email: ${update.user.email})`
        );
        return true;
      }
      return false;
...
const getBonusSessions = async (): Promise<BonusSession[]> => {
  try {
      ...
      if (parts.length >= 4) {
        ...
        if (user && utcTimestampStr && localTimestampStr) {
          ...
          if (!isNaN(sessionDate.getTime()) && localDatePart) {
            ...
            console.warn(
              `Invalid date or local timestamp at line ${index + 2}:`,
              { utcTimestampStr, localTimestampStr, sessionDate }
            );
          }
        } else {
    ...
    console.log(
      `Parsed ${results.length} unique sessions from ${lines.length} lines.`
    );
    return results;
  } catch (error) {
...
const getLiveCompetitionUpdates = async (
  ...
  // We do NOT filter by end date here, because if a page is modified AFTER the competition,
  // we still need to fetch it to find the valid versions within the competition window.
  const cql = `space = "SE" AND type = "page" AND lastModified >= "${start}"`;

  console.info(`[LIVE] Fetching pages with CQL: ${cql}`);

  const allPages = await searchPagesV1(cql, 1000);
  const pages = allPages.filter((page: any) => !shouldExcludePage(page.title));

  console.info(
    `[LIVE] Found ${pages.length} pages updated during the competition (filtered from ${allPages.length}).`
  );
  ...
  const updatesPromises = pages.map(async (page: any) => {
    ...
    const relevantVersions = versions.filter((v: any) => {
      ...
      return (
        d >= contests[0].start &&
        d < contests[contests.length - 1].end &&
        v.number > 1
      );
    });

    console.debug(
      `[LIVE] Page ${page.id}: ${relevantVersions.length} qualifying edits found.`
    );

    const versionUpdates = await Promise.all(
      relevantVersions.map(async (v: any) => {
        try {
          ...
          if (v.number > 1) {
            try {
              const currentVersion = await getPageVersionV2ByNumber(
                page.id,
                v.number
              );
              const prevVersion = await getPageVersionV2ByNumber(
                page.id,
                v.number - 1
              );

              // Trace specific user for troubleshooting
              if (
                user.email === "richardrose@newrelic.com" ||
                user.name.includes("Richard Rose")
              ) {
                console.log(
                  `[TRACE] Richard Rose edit: Page "${page.title}" (id: ${page.id}) v${v.number}, timestamp: ${v.createdAt}`
                );
              }

              const hasBody =
                currentVersion?.body?.storage?.value &&
                prevVersion?.body?.storage?.value;
              ...
              if (
                ...
                console.warn(
                  `[TRACE] Richard Rose edit missing body content: v${v.number} on ${page.title}`
                );
                console.log(
                  "Current body present:",
                  !!currentVersion?.body?.storage?.value,
                  "Prev body present:",
                  !!prevVersion?.body?.storage?.value
                );
              ...
              if (hasBody) {
                ...
                console.log(
                  `[Points] ${user.name} created p${page.id}-v${v.number} at ${v.createdAt}: ${currentText.length} - ${prevText.length} = ${points}`
                );
              }
            } catch (err) {
              console.error(
                `[Points] Failed to calc diff: ${user.name} created p${page.id}-v${v.number} at ${v.createdAt}`,
                err
              );
            }
          }
          ...
          return {
            ...
            pageUrl: `${siteUrl}/wiki/spaces/${page.spaceId || "SE"}/pages/${
              page.id
            }`,
            user,
  ...
  const flatUpdates = nested.flat() as Omit<
    PageUpdate,
    "multiplier" | "bonusType"
  >[];
...
export const getCompetitionUpdates = async (): Promise<PageUpdate[]> => {
  ...
  return isForge
    ? getLiveCompetitionUpdates(config)
    : getMockCompetitionUpdates(config);
};
...
const getMockCompetitionUpdates = async (
  config: ContestConfig
): Promise<PageUpdate[]> => {
  const sessions = await getMockBonusSessions();
  const updates = generateMockUpdates(sessions, config);
  const filteredUpdates = updates.filter(
    (update) => !shouldExcludePage(update.pageTitle)
  );
  return applyBonuses(filteredUpdates, sessions, config.criticalContent);
};
